...
On peut lui préciser le chemin de la version que l'on veut utiliser une certaine version plutôt que celle par défaut. Si une version de `Python` est déjà chargée, il faut relancer la session `R` pour que la modification soit appliqué. Dans notre exemple nous allons utiliser l'environnement conda `finistR2020` qui est décrit dans le fichier `binder/environment.yml`.
```{r}
library(reticulate)
use_condaenv("finistR2020")
