...
Le package torch est une librairie de calcul tensoriel en R, utilisant 
les fonctions de libtorch. Il peut aussi faire de la différentiation 
automatique (autograd). Le package est similaire à la librairie PyTorch 
en Python.
...
Cependant, il faut peut-être installer/réinstaller des dépendances mal 
gérées. On a dû réinstaller rlang et bit64 manuellement.

Sur le site, des exemples d'application sont présentés avec le dataset 
mnist. Pour l'instant, il semble que le package soit encore en 
développement et les datasets mnist ne sont pas présents dans la branche 
master du dépôt.

## Le package R rTorch
La librairie de deep learning PyTorch (https://pytorch.org) sous Python est très populaire. Le package rTorch est un package R qui fait l'interface entre R et PyTorch.

Remarque : selon  http://eric.univ-lyon2.fr/~ricco/tanagra/fichiers/fr_Tanagra_PyTorch_PMC.pdf, le plus compliqué dans l'utilisation primaire de PyTorch, c’est la nécessité de transformer les matrices et vecteurs de données (Pandas ou Numpy) en tensor reconnus par PyTorch.
