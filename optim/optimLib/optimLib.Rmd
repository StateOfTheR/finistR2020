---
title: "Outils d'optimisation: optimLib"
subtitle: "Interfaçage d'une bibliothèque C++ d'optimisation, alternative à `optim`"
author: "Saint-Clair Chabert-Liddell, Julien Chiquet"
date: "24/08/2020"
...
## Motivation
  ...
  1. Apprendre à interfacer une librarie C++ avec R en l'intégrant dans un package
    ...
    - etc.
  2. Disposer des implémentation alternative des méthodes d'optimisation classique 
  3. Disposer d'implémentations de méthodes non présentes dans `optim`, comme ADAM

## La bibliothèque `optimLib`

Il s'agit d'une [bibliothèque C++ développée par Keith O'hara ](https://github.com/kthohr/optim)

Les algorithmes d'optimisation implémentés incluent
...
Nous avons proposé le [package R **optimLibR** intégrant de manière minimale la librarie C++](https://github.com/StateOfTheR/optimLibR)

```{r}
library(optimLibR)
```

Pour ce faire, les code source de la bibliothèque **optimLib** sont intégrés sous forme de ".hpp" (header only") dans le répertoire `inst/include` du package. Il suffit ensuite de déclarer correctement les variables de compilation dans les fichiers `Makevars` du répertoire ` src`.
