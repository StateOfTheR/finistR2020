...
- `across()`, `relocate()`, `rename()`, `select()`, and `pull()` : ces verbes utilisent la sélection tidy (tidy selection) et permettent de choisir facilement des variables en se basant sur leur position, leur nom ou leur type  (e.g. `starts_with("x")` or `is.numeric()`).
...
Mais le fait de devoir spécifier dès le départ le nom de colonne est désagréable et peu informatif. L'opérateur `:=` permet de créer des variables *nommées de façon dynamique* dans des verbes du tidyverse
...
On peut chercher à créer une fonction représentant les captures de deux navires pour 3 espèces (la sole, le merlu, la langoustine).
...
plot_eff.sev.vessels <- function(data,vessels,species,unit){
  data %>% 
    dplyr::select(aggreg_level:lati,contains(paste0(.env$unit,"_",.env$species))) %>% # on va chercher l'unité et les espèces dans l'environnement 
    pivot_longer(contains(paste0(.env$unit,"_",.env$species)),names_to = "species",values_to = unit) %>%  # .env$unit ne marche pas ?
    mutate(species = str_remove(species, paste0(.env$unit,"_"))) %>%
    group_by(layer,long,lati,species) %>%
    summarise(value = mean({{unit}})) %>%
    ggplot() +
    geom_point(aes(x=long,y=lati,col = log(value+1)),size = 0.5,shape=15) +
    geom_sf(data = world)+
...
data_2 <- VMS_data
vessels_2 <- c("16629","37581")
species_2 <- c("Solea_solea","Merluccius_merluccius","Nephrops_norvegicus")
unit_2 <-"CPUE"
plot_eff.sev.vessels(data_2,vessels_2,species_2,unit_2)
