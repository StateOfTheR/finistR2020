...
- `arrange()`, `count()`, `filter()`, `group_by()`, `mutate()`, et `summarise()` : ces verbes utilisent le masquage de données (data masking). De cette façon, les variables dans des données peuvent être utilisées comme si elles étaient dans l'environnement. Plus besoin d'écrire `df$var` mais simplement `var` dans le verbe dplyr.
...
La sélection tidy est un outil complémentaire qui facilite le travaille sur les colonnes d'un jeu de données.
