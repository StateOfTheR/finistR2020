...
sigma <- 20
noisy <- out_sir
noisy$S <- pmax(out_sir$S + rnorm(length(out_sir$time), 0, sigma), 0)
noisy$I <- pmax(out_sir$I + rnorm(length(out_sir$time), 0, sigma), 0)
noisy$R <- pmax(out_sir$R + rnorm(length(out_sir$time), 0, sigma), 0)
...
basis_dim1 <- create.bspline.basis(c(0, max(times)), nbasis, 
                                   norder, breaks = knots)
...
pcode.result <- pcode(
  data = observ,
  time = times,
  ode.model = closed.sir.model,
  par.initial = c(100, 1),
  par.names = c('beta', 'gamma'),
  state.names = c('S', 'I', 'R'),
  basis.list = basis,
  lambda = 1e2
)
