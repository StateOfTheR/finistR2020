...
```{r}
plot_data <- data.frame(out) %>% 
    rename(numeric = X1) %>% 
    mutate(analytic = exp(-time)) %>% 
    pivot_longer(cols = -time, names_to = "type", values_to = "y")
...
sigma = 20
noisy <- out;
noisy$S  <- pmax(out$S  + rnorm(length(times),0,sigma),0)
noisy$I  <- pmax(out$I  + rnorm(length(times),0,sigma),0)
noisy$R  <- pmax(out$R  + rnorm(length(times),0,sigma),0)
