...
On remarque que ça n'est pas centré : on a normalisé avec la moyenne et la variance du jeu de donnée d'entraînement.
...
grid <-
  spec_rf %>%
  parameters() %>%
  update(mtry = finalize(mtry(), select(juiced, -Salary))) %>%
...
```{r tune, cache=TRUE, cache.lazy=FALSE}
...
On ajuste un modèle avec les meilleurs paramètres.
...
Calcule des métriques ce dernier modèle.

```{r}
collect_metrics(wkf_rf_fit)
```

On rapelle que le RMSE pour la regression linéaire vaut `r  my_metrics(df_test_lr, truth = Salary, estimate = .pred)$.estimate[1]`.
