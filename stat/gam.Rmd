...
On veut construire une base de $k$ fonctions splines $(s_1, \ldots, s_k)$. Pour ceci, on définit $k$ noeuds, par défaut les extrémités correspondent à $x_{min}$ et $x_{max}$.

Chaque fonction $s_i$ vaut 1 au noeud $i$ et 0 aux autres noeuds. En outre, chaque fonction spline (cublique) est le recollement de $k$ polynômes de degré 3 assurant une fonction  de classe $C^2$  sur l'intervalle, comme illustré ci-dessous (extrait du livre de S. Wood).



![](img/spline.jpg)

## Exemples de code avec le package `mgcv`
